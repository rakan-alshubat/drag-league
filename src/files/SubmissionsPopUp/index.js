...
export default function SubmissionsPopup({
    ...
    // Check if current player has already submitted this week
    const getCurrentWeekSubmission = () => {
        if (!leagueData?.lgSubmissions || !playerData?.plEmail) return null;
        
        const playerEmail = String(playerData.plEmail).toLowerCase();
        const submissions = leagueData.lgSubmissions || [];
        
        // lgSubmissions format: ["QueenName|player@email.com", ...]
        for (const sub of submissions) {
            const parts = String(sub || '').split('|').map(s => s.trim());
            if (parts.length === 2) {
                const [queenName, submitterEmail] = parts;
                if (String(submitterEmail).toLowerCase() === playerEmail) {
                    return queenName;
                }
            }
        }
    ...
    const handleSubmit = async () => {
        ...
        if (version === "Submissions") {
            serverLogInfo('Weekly picks submission started', { leagueId: leagueData?.id, userId: userData?.id, playerName: playerData?.plName });
            try {
                const dl = leagueData?.lgDeadline ? new Date(leagueData.lgDeadline) : null;
                if (dl && Date.now() >= dl.getTime()) {
                    serverLogWarn('Weekly picks submission rejected: deadline passed', { leagueId: leagueData?.id, userId: userData?.id, deadline: dl.toISOString() });
            ...
            if (playerData?.id && Object.keys(playerUpdates).length > 0) {
                try {
                    ...
                    if (!isDemo) {
                        ...
                        serverLogInfo('Player swap performed', { leagueId: leagueData?.id, playerId: playerData.id, firstSwap: firstSwap, secondSwap: secondSwap });
                        ...
                        if (playerUpdates.plRankings && leagueData?.id) {
                            ...
                            serverLogInfo('League history updated with swap', { leagueId: leagueData.id, playerName: playerData.plName, firstSwap: firstSwap, secondSwap: secondSwap });
                        }
                    } else {
                    ...
                    serverLogError('Error updating player swap', { error: error.message, leagueId: leagueData?.id, playerId: playerData?.id });
            ...
            if (selected.length > 0 && leagueData?.id && userData?.id) {
                try {
                    ...
                    try {
                        ...
                        serverLogWarn('Could not fetch latest league before updating submissions', { error: fetchErr.message });
                    }
                    ...
                    if (!isDemo) {
                        ...
                        serverLogInfo('Weekly picks submitted to league', { leagueId: leagueData.id, userId: userEmail, playerName: playerData?.plName, picksCount: selected.length, action: actionText });
                    } else {
                    ...
                    serverLogError('Error updating league submissions', { error: error.message, leagueId: leagueData?.id, userId: userData?.id });
        ...
        if (version === "Weekly Results" && isFinalEpisode && leagueData?.id) {
            serverLogInfo('Final episode submission started', { leagueId: leagueData.id, adminName: userData?.name || userData?.id });
            ...
            if (uniqueProvided.length !== remainingCount) {
                serverLogWarn('Final episode submission validation failed', { leagueId: leagueData.id, provided: uniqueProvided.length, required: remainingCount });
            ...
            try {
                ...
                if (Array.isArray(allPlayers)) {
                    ...
                    serverLogInfo('Final episode: all player winners updated', { leagueId: leagueData.id, playerCount: allPlayers.length });
                }
                ...
                if (!isDemo) {
                    ...
                    serverLogInfo('Final episode completed', { leagueId: leagueData.id, winner: firstPlaceDisplay, bonusCategories: bonusCategoryRows.length });
                } else {
        ...
        if (leagueData?.id && version === "Weekly Results" && !isFinalEpisode) {
            serverLogInfo('Weekly results submission started', { leagueId: leagueData.id, adminName: userData?.name || userData?.id, challengeWinners: challengeWinners.length, lipSyncWinners: lipSyncWinners.length, eliminated: eliminatedQueens.length });
            try {
                // Process lgSubmissions and map to player plWinners
                const submissions = leagueData.lgSubmissions || [];
                const submissionMap = {};
                ...
                if (Array.isArray(leaguePlayers) && leaguePlayers.length > 0) {
                    ...
                    if (missing.length > 0) {
                        const names = missing.map(p => p.plName || p.plEmail || p.id).join(', ');
                        serverLogWarn('Weekly results submitted with missing player picks', { leagueId: leagueData.id, missingCount: missing.length, missingPlayers: names });
                ...
                if (Array.isArray(allPlayers) && allPlayers.length > 0) {
                    ...
                    await Promise.all(updatePromises);
                    serverLogInfo('Weekly results: all player winners updated', { leagueId: leagueData.id, playerCount: allPlayers.length });
                } else {
                    serverLogWarn('No players array found in leagueData or array is empty');
                }
                ...
                serverLogError('Error updating league weekly results', { error: error.message, leagueId: leagueData?.id });
                        ...
                        {(Number(leagueData?.lgChallengePoints || 0) > 0) && (
                            <>
                                ...
                                <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5, mt: 1 }}>
                                    ...
                                    {(() => {
                                        const currentSubmission = getCurrentWeekSubmission();
                                        if (currentSubmission) {
                                            return (
                                                <Alert severity="success">
                                                    You already submitted your pick this week: <strong>{currentSubmission}</strong>. 
                                                    You can overwrite your pick by selecting a different queen and submitting again.
                                                </Alert>
                                            );
                                        } else {
                                            return (
                                                <Alert severity="info">
                                                    Your pick will not be visible to you or others in the submission tab until the results are submitted so nobody will see who you picked until everyone picks.
                                                </Alert>
                                            );
                                        }
                                    })()}
